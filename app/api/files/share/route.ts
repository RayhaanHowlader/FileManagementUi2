...
import { sendShareEmail } from "@/lib/share-mailer"
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { fileId, fileName, fileUrl, fileType, recipientEmail, permission, expiry, message } = await req.json()
    ...
    return NextResponse.json({ message: "Share email sent", shareLink })
  } catch (err) {
